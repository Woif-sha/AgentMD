...
4. Run `scripts/validate-links.ps1`, which must accept both current AgentMD links and current managed copies while continuing to require the repository's own `CLAUDE.md` links. Do not deliver the change until installation and validation both pass.
