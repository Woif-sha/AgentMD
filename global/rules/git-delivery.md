...
5. Wait for the workflow, then verify its conclusion, Release tag and title, downloadable artifact identity and version, and remote branch/tag alignment. Existing versions are immutable: a rerun must not move the tag or replace an asset with different bytes.
