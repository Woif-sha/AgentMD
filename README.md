...
Add a row when an upstream repository enters consideration, pin the reviewed revision, and update its status rather than removing its history. Link the integrating AgentMD commit when it exists; keep research notes local. Do not assign provenance from textual similarity alone.
